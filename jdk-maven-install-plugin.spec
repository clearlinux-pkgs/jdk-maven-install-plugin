...
License  : Apache-2.0
Requires: jdk-maven-install-plugin-data
...
%package data
Summary: data components for the jdk-maven-install-plugin package.
Group: Data

%description data
data components for the jdk-maven-install-plugin package.


%prep
%setup -q -n maven-install-plugin-2.5.2

python3 /usr/share/java-utils/pom_editor.py pom_add_dep        org.apache.maven:maven-core::test
...
%install
xmvn-install  -R .xmvn-reactor -n maven-install-plugin -d %{buildroot}
...
%files data
%defattr(-,root,root,-)
/usr/share/java/maven-install-plugin/maven-install-plugin.jar
/usr/share/maven-metadata/maven-install-plugin.xml
/usr/share/maven-poms/maven-install-plugin/maven-install-plugin.pom
